...
class HouseholdFixtureMixin:
    @classmethod
    def setUpTestData(cls):
        cls.member_a = HouseholdMember.objects.create(name="Alex")
        cls.member_b = HouseholdMember.objects.create(name="Sam")


class ChoreModelValidationTests(HouseholdFixtureMixin, TestCase):
    ...
    def test_one_off_requires_due_date(self):
        chore = Chore(name="Pay bill", assigned_to=self.member_a, recurrence_type=Chore.RecurrenceType.ONE_OFF)
        with self.assertRaises(ValidationError):
            chore.full_clean()

    def test_daily_chore_is_valid_without_optional_fields(self):
        chore = Chore(name="Exercise", assigned_to=self.member_a, recurrence_type=Chore.RecurrenceType.DAILY)
        chore.full_clean()


class TodayStatusTests(HouseholdFixtureMixin, TestCase):
    def test_daily_due_if_not_completed_today(self):
        chore = Chore.objects.create(name="Exercise", assigned_to=self.member_a, recurrence_type=Chore.RecurrenceType.DAILY)
    ...
    def test_weekly_target_due_until_target_reached(self):
        ...
        Completion.objects.create(chore=chore, completed_on=today, completed_by=self.member_a)
        status = get_today_status(chore, today=today)
        self.assertTrue(status.is_due)

        Completion.objects.create(chore=chore, completed_on=today - timedelta(days=1), completed_by=self.member_b)
        status = get_today_status(chore, today=today)
...
class StreakLogicTests(HouseholdFixtureMixin, TestCase):
    def test_daily_streak_allows_one_grace_gap(self):
        today = date(2026, 9, 7)
        chore = Chore.objects.create(name="Exercise", assigned_to=self.member_a)
        Completion.objects.create(chore=chore, completed_on=today, completed_by=self.member_a)
        Completion.objects.create(chore=chore, completed_on=today - timedelta(days=1), completed_by=self.member_b)
        Completion.objects.create(chore=chore, completed_on=today - timedelta(days=3), completed_by=self.member_a)
    ...
    def test_weekly_target_streak_allows_one_grace_week(self):
        ...
        Completion.objects.create(chore=chore, completed_on=current_week, completed_by=self.member_a)
        Completion.objects.create(chore=chore, completed_on=current_week + timedelta(days=1), completed_by=self.member_b)
        Completion.objects.create(chore=chore, completed_on=older_week, completed_by=self.member_a)
        Completion.objects.create(chore=chore, completed_on=older_week + timedelta(days=2), completed_by=self.member_b)
    ...
    def test_weekly_target_streak_resets_after_second_miss(self):
        ...
        Completion.objects.create(chore=chore, completed_on=old_week, completed_by=self.member_a)
        self.assertEqual(get_current_streak(chore, today=today), 0)


class ChoreFlowTests(HouseholdFixtureMixin, TestCase):
    def test_quick_add_creates_chore(self):
        response = self.client.post(
            reverse("chores:list"),
            {"name": "Laundry", "assigned_to": self.member_a.id},
        )
        self.assertEqual(response.status_code, 302)
        chore = Chore.objects.get(name="Laundry")
        self.assertEqual(chore.recurrence_type, Chore.RecurrenceType.DAILY)
        self.assertEqual(chore.assigned_to, self.member_a)

    def test_edit_and_archive_chore(self):
        chore = Chore.objects.create(name="Old name", assigned_to=self.member_a)
        ...
        self.assertEqual(chore.assigned_to, self.member_b)
    ...
    def test_mark_done_with_backdate_and_undo(self):
        chore = Chore.objects.create(name="Dishes", assigned_to=self.member_a)
        complete_response = self.client.post(
            reverse("chores:complete", args=[chore.id]),
            {
                "completed_on": "2026-09-01",
                "completed_by": self.member_b.id,
        ...
        self.assertEqual(completion.completed_by, self.member_b)
    ...
    def test_today_view_shows_overdue_one_off(self):
        Chore.objects.create(name="Archived", is_archived=True, assigned_to=self.member_a)
        Chore.objects.create(
            name="Renew card",
            assigned_to=self.member_b,
            recurrence_type=Chore.RecurrenceType.ONE_OFF,
